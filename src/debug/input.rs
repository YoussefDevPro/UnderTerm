    ...
    match key.code {
        ...
        KeyCode::Char('t') => {
            if !game_state.is_drawing_teleport_zone {
                ...
                game_state.teleport_zone_start_coords = Some((game_state.player.x, game_state.player.y));
        ...
        KeyCode::Enter => {
            if game_state.is_drawing_select_box {
                if let Some((start_x, start_y)) = game_state.select_box_start_coords {
                    ...
                    let (width, height) = (max_x.saturating_sub(min_x).saturating_add(1), max_y.saturating_sub(min_y).saturating_add(1));
                ...
                if let Some((start_x, start_y)) = game_state.teleport_zone_start_coords {
                    ...
                    if width > 0 && height > 0 {
                        ...
                        game_state.message_animation_start_time = Instant::now();
                        game_state.animated_message_content.clear();
                        game_state.teleport_zone_start_coords = Some((min_x, min_y));
                        game_state.select_box_start_coords = Some((width, height));
                    }
                }
