...
use under_term::load_sprite_asset_str;

const FACE_SPRITES: &[&str] = &[
...
impl Editor {
    fn new() -> Self {
        let faces = FACE_SPRITES.iter().map(|s| s.to_string()).collect();
        let enemies = ENEMY_SPRITES.iter().map(|s| s.to_string()).collect();
