        ...
        match event {
            Event::Key(key) => {
                ...
                if game_state.show_enemy_ansi {
                    if key.kind == event::KeyEventKind::Press {
                        game_state.show_enemy_ansi = false;
                    }
                    continue;
                }
            ...
            Event::Resize(_width, _height) => {
                ...
                // However, we could log it for debugging or update game_state if needed.
                // For now, just acknowledging the event.
                // println!("Resized to {}x{}", width, height); // For debugging
            }
            _ => {
                // Ignore other event types like mouse events for now
            }
        }
    }
    Ok(false)
